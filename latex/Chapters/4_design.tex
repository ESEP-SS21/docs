...
Die Architektur der Software für eine \gls{anlage} ist in Abbildung~\ref{fig:cmp} abgebildet.
...
\subsubsection{cnnMngmnt}
Das cnnMngmnt namespace enthält Wrapper für QNX-Services.
Ein QnxChannel stellt Methoden zum Empfangen von Nachrichten zur Verfügung.
Dieser kann optional mit einem Namen erstellt werden, um von einer anderen \gls{anlage} auf
diesen zugreifen zu können.
Anschließend kann mithilfe der Methode msg\_receive eine Nachricht blockierend empfangen
werden.
Mithilfe einer QnxConnection werden Nachrichten an einen Channel verschickt.
Dieser wird entweder mit der chid des Channels oder mit dem dessen Namen erstellt.
\FloatBarrier
